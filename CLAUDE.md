...
**Local mode** is four patches plus loader UI, and it needs all of them. `fetchConfigFromWorker` throws (boot() already falls back to the localStorage cache, which is exactly the wanted behaviour) and `pushConfigToWorker` resolves as a no-op. The write side is not optional: hiding a device and editing a custom dashboard push to the server on their own, so without it a "local" browser would still mutate the shared layout. The loader hides the save button, which would otherwise report success for a write that never happened.

**Gating the two transport functions is not sufficient.** `reset-cfg` issues its own `fetch(API_CONFIG, {method:'DELETE'})` instead of going through `pushConfigToWorker`, so it slipped straight past the write gate — a local-mode browser would have wiped the shared config off the hub. That is what `local-only-reset-spares-hub-config` fixes, and it is the reason to grep for direct `fetch(API_CONFIG` / `fetch(API_HUB` call sites rather than trusting that all traffic flows through the two helpers. If upstream adds another direct call, local mode leaks again.
